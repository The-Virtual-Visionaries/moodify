...
const dayUsermood = async (
  ...
  try {
    ...
    if (!usermood) {
      return res.status(200).json({
        data: { valid: false, mood: { date: "", entry: "", mood: "" } },
      })
    }
    ...
    return res
      .status(200)
      .json({ data: { valid: false, mood: { date: "", entry: "", mood: "" } } })
  } catch (error) {
  ...
  try {
    ...
    if (!usermoods) {
      ...
      if (yesterdayMood) {
        ...
        return res
          .status(200)
          .json({ message: "Mood added, streak increased", mood: mood })
      } else {
        ...
        return res
          .status(200)
          .json({ message: "Mood add, streak reset", mood: mood })
      }
    }
  } catch (error) {
...
const getStreak = async (
  ...
  try {
    ...
    const yesterdayDate =
      ...
      "-" +
      yesterday.getDate()
    const yesterdayMood = usermoods.moods.find(
      (mood) => mood.date === yesterdayDate
    )
    if (!yesterdayMood) {
      if (!todayMood) {
        const newStreak = await Usermood.updateOne(
